...
namespace YueHuan.Systemaitc
{
    ...
    public class RegistryManager
    {
        ...
        /// <br/>主要包括：
        /// <br/>
        /// <br/>1. <paramref name="ClassesRoot"/>         对应于HKEY_CLASSES_ROOT主键 
        /// <br/>2. <paramref name="CurrentUser"/>         对应于HKEY_CURRENT_USER主键
        /// <br/>3. <paramref name="LocalMachine"/>        对应于 HKEY_LOCAL_MACHINE主键
        /// <br/>4. <paramref name="User"/>                对应于 HKEY_USER主键
        /// <br/>5. <paramref name="CurrentConfig"/>       对应于HEKY_CURRENT_CONFIG主键
        /// <br/>6. <paramref name="DynDa"/>               对应于HKEY_DYN_DATA主键
        /// <br/>7. <paramref name="PerformanceData"/>     对应于HKEY_PERFORMANCE_DATA主键
        /// <br/>
        /// <br/>版本:1.0
        /// </summary>
        ...
        /// <br/>
        /// <br/>主要包括：
        /// <br/>1.<paramref name="RegistryValueKind.Unknown"/>
        /// <br/>2.<paramref name="RegistryValueKind.String"/>
        /// <br/>3.<paramref name="RegistryValueKind.ExpandString"/>
        /// <br/>4.<paramref name="RegistryValueKind.Binary"/>
        /// <br/>5.<paramref name="RegistryValueKind.DWord"/>
        /// <br/>6.<paramref name="RegistryValueKind.MultiString"/>
        /// <br/>7.<paramref name="RegistryValueKind.QWord"/>
        /// <br/>
        /// <br/>版本:1.0
        /// </summary>
        ...
        public class Register
        {
            ...
            #region 读取注册表项
            /// <summary>
            /// 读取注册表项，默认是在注册表基项HKEY_LOCAL_MACHINE下判断（请先设置SubKey属性）
            /// <br/>虚方法，子类可进行重写
            /// <br/>默认读取HKEY_LOCAL_MACHINE\\SOFTWARE\\
            /// </summary>
            /// <returns>返回基项下的所有子项名</returns>
            public virtual string[] ReadSubKey()
            {
                string[] readSubKey;
                ...
                try
                {
                    ...
                    readSubKey = sKey.GetSubKeyNames();
                }
                catch (Exception ex)
                {
            ...
            /// 读取注册表项，默认是在注册表基项HKEY_LOCAL_MACHINE下判断（请先设置SubKey属性）
            /// <br/>虚方法，子类可进行重写
            /// <br/>例：如果设置了SubKey为HiGame\\SKY，则读取Game\\SKY，默认读取HKEY_LOCAL_MACHINE\\SOFTWARE\\
            /// </summary>
            /// <returns>返回基项下的所有子项名</returns>
            public virtual string[] ReadSubKey(string subKey)
            {
                string[] readSubKey;
            ...
            /// 读取注册表项，默认是在注册表基项HKEY_LOCAL_MACHINE下判断（请先设置SubKey属性）
            /// <br/>虚方法，子类可进行重写
            /// <br/>例：如regDomain是HKEY_CLASSES_ROOT，则读取HKEY_CLASSES_ROOT\\SubKey
            /// </summary>
            /// <returns>返回基项下的所有子项名</returns>
            public virtual string[] ReadSubKey(string subKey, RegDomain regDomain)
            {
                string[] readSubKey;
            ...
            #region 设置键值内容
            /// <summary>
            /// 设置指定的键值内容，不指定内容数据类型,键值名默认为程序名，如果不存在则先创建再设置键值
            /// </summary>
            ...
            public virtual bool WriteRegeditKey(object content)
            {
                // 返回结果
                bool result = false;
                string regEditKey = AppDomain.CurrentDomain.FriendlyName;
            ...
            public virtual object ReadRegeditKey(string name, string subKey)
            {
                ...
                {
                    ///判断键值是否存在
                    if (IsRegeditKeyExist(name, subKey))
                    {
                        obj = key.GetValue(name);
                    }
                }
